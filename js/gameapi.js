	...
	module.prototype.initializeFirebase = function() {
		...
		M.promise = new Promise(function(resolve, reject) {
			...
			faZepto.getScript(
				...
				function() {
					faZepto.getScript("https://www.gstatic.com/firebasejs/5.9.2/firebase-firestore.js",
						function() {
							faZepto.getScript(
								"https://www.gstatic.com/firebasejs/5.9.2/firebase-database.js",
								function() {

									// Initialize Firebase
									var config = {
										apiKey:
											"AIzaSyAN2Zj_J4pObQIbXxYYDYatv-VkMvxLoVQ",
									...
									M.firestore = firebase.firestore();
						...
						function() {
							reject("Firebase Firestore not ready");
						}
					);
				},
				function() {
...
fg_api.prototype.lsgModule = function() {
	...
	module.prototype.init = function() {
		...
		this.is_floor_price_based = (now.getUTCHours() % 2 > 0);
		...
		if (!M.promise) {
			M.promise = new Promise(function(resolve, reject) {
				Promise.resolve(M.getLocale())
					...
					.then(function() {
						// initialize firebase and get data for randomized or optimized pricerule selection
						if (M.isSamplingAllRules()) {
							return M.pickRandomRule().then(resolve, reject);
						} else {
							return M.pickDynamicRule().then(resolve, reject);
						}
					})
					.catch(function(err) {
	...
	module.prototype.isFloorPriceBased = function(setter) {
		if (typeof setter !== "undefined") {
			M.is_floor_price_based = !!setter;
		}
		return !!M.is_floor_price_based;
	};
